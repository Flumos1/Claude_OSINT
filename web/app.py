...
# ── Режим и защита ────────────────────────────────────────────────────────────
# READ_ONLY: на Vercel (serverless) файловая система только для чтения → «Сохранить
#   в кейс» недоступно. Vercel сам ставит env VERCEL=1; можно форсить READ_ONLY=1.
# ACCESS_PASSWORD: если задан — простой парольный гейт (подписанная cookie). Без него
#   доступ открыт (локальный режим).
READ_ONLY = bool(os.getenv("VERCEL") or os.getenv("READ_ONLY"))
ACCESS_PASSWORD = os.getenv("ACCESS_PASSWORD") or ""
_AUTH_COOKIE = "osint_auth"
_OPEN_PATHS = ("/api/login", "/api/docs", "/api/openapi.json")


def _auth_token() -> str:
    return hmac.new(ACCESS_PASSWORD.encode(), b"osint-authed", hashlib.sha256).hexdigest()
...
def _authed(request: Request) -> bool:
    ...
    return hmac.compare_digest(tok, _auth_token())


@app.middleware("http")
async def _gate(request: Request, call_next):
    """Пропускает статику, логин и индекс; остальные /api/* — только с валидной cookie."""
    path = request.url.path
    if ACCESS_PASSWORD and path.startswith("/api/") and path not in _OPEN_PATHS:
...
def api_login(req: LoginReq):
    ...
    resp.set_cookie(_AUTH_COOKIE, _auth_token(), httponly=True, samesite="lax",
                    secure=bool(os.getenv("VERCEL")), max_age=60 * 60 * 24 * 7)
    return resp
...
def api_case_save(req: CaseSaveReq):
    if READ_ONLY:
        raise HTTPException(403, "Read-only режим (serverless): сохранение в кейс доступно "
                                 "только в локальном запуске. Используйте экспорт MD/DOCX/PDF.")
