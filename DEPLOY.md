...
| `amass` | Amass (OWASP) | domain | **выключен по умолчанию**, `AMASS_ENABLE=1` |

### Amass: выключен по умолчанию

В отличие от остальных, Amass — клиент-серверная система с локальной графовой
БД (`amass engine` фоновым сервисом + `amass enum` отправляет цель + `amass subs`
читает результаты) — полный цикл занимает **минуты**, не секунды. Включённый
по умолчанию он сломал бы обычную цепочку из 9 domain-энричеров (ровно та
проблема, что уже ловили с таймаутами). Включить для разового глубокого скана
(из контейнера, `../scripts` — рабочая директория `web/`):
```bash
docker compose exec osint bash -c "cd ../scripts && AMASS_ENABLE=1 python enrich.py domain example.com"
```
Тюнинг: `AMASS_ENUM_TIMEOUT_MIN` (по умолчанию 3 мин), `AMASS_ENGINE_WAIT` (запуск engine).
