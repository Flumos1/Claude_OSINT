...
> 💾 **Install-подсказки** (метод + команда) для рабочих лошадок —
> [`curated-tools.json`](curated-tools.json) (наша компиляция; команды из официальных
> источников каждой тулзы). Показываются в веб-платформе (раздел «Инструменты»).
