"""
Энричер email → регистрации, БЕЗОПАСНЫЙ вариант (официальные публичные API, не формы).

Использует только штатные публичные сигналы существования аккаунта:
  • Gravatar/Libravatar — штатный публичный чек «есть ли аватар» (?d=404): 200=есть, 404=нет.

Это лёгкий, всегда-безопасный слой. Более глубокая (и рискованнее по ToS — см. коммент в
holehe_enr.py) проверка ~120 сервисов через их signup/password-reset формы — отдельный
энричер `holehe_accounts` (holehe_enr.py), включён по умолчанию по решению оператора.

Возвращает: узел email, факты о найденных публичных профилях-по-email.
"""
import hashlib
